...
def poisson_disc_samples_free(width, height, r, k=5, distance=euclidean_distance, random=random, segments=5, inputRaster=[]):
	global radius, center, dynamic_ratio

	if len(inputRaster) == 0:
		raise Exception('inputRaster cannot be empty.')
...
def genMesh():
	...
	if generateBlob:
		...
		print(letter)
		samples = poisson_disc_samples_free(width=xsize, height=ysize, r=10, k=k, segments=perimeterSegments, inputRaster=letter)
		...
		startingR = perimeterSegments / 10
		startTime = int(round(time.time() * 1000))
		samples = poisson_disc_samples_free(width=xsize, height=ysize, r=20, k=k, segments=perimeterSegments)
		endTime = int(round(time.time() * 1000))
