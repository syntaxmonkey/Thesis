...
def calcGST(inputIMG, w):
	...
	imgDiffX = filters.Sobel(img, cv.CV_32F, 1, 0, 3)
	imgDiffY = filters.Sobel(img, cv.CV_32F, 0, 1, 3)
	imgDiffXY = np.multiply(imgDiffX, imgDiffY)

	imgDiffXX = np.multiply(imgDiffX, imgDiffX)
	imgDiffYY = np.multiply(imgDiffY, imgDiffY)
