...
def sizeInit(base_image_path):
    ...
    #img = load_img(base_image_path, grayscale=True)
    img = generateImage(7,7)
    print(img)
    #print(np.shape(img))
    ...
    print("gradient: ", np.shape(gradients))
    print("gradient average: ")
    print( (gradients[0] + gradients[1])/2.0 )
    print("gradients:")
    print(gradients)
...
if False:
    ...
    #print(imageArray)
    input_folder = '/Users/hengsun/PycharmProjects/ThesisTest/img'
    input_image = 'animal-blue-pattern-danger-41180_scaled.jpg';
    baseImage = input_folder + '/' + input_image
    sizeInit(baseImage)
    img_nrows = 224
    img_ncols = 224

    # baseimage = K.placeholder((1, 3, img_nrows, img_ncols))
    # startimage = K.placeholder((1, 3, img_nrows, img_ncols))
    # combination = K.placeholder((1, 3, img_nrows, img_ncols))
    # input_tensor = K.concatenate([startimage, baseimage, combination], axis=0)
    # model = vgg19.VGG19(input_tensor=input_tensor,
    #                     weights='imagenet', include_top=False)
